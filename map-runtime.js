...
function getMapConfig() {
  ...
  return {
    defaultProvider: amapKey ? "amap" : googleKey ? "google" : "none",
    providers: {
...
async function handleMapPlan(req, res) {
  try {
    ...
    if (provider === "none") {
      sendJson(res, 503, { ok: false, error: "missing map api key" });
      return;
    }
    ...
    if (!entries.length) {
      sendJson(res, 400, { ok: false, error: "missing itinerary points" });
      return;
    }
...
async function geocodeEntry(provider, entry, config) {
  const point = provider === "google"
    ? await geocodeGoogle(entry.query, config.providers.google.key)
    : await geocodeAmap(entry.query, config.providers.amap.key);
...
async function routeSegment(provider, from, to, mode, config) {
  ...
  try {
    return provider === "google"
      ? await routeGoogle(from, to, mode, config.providers.google.key)
      : await routeAmap(from, to, mode, config.providers.amap.key);
  } catch {
...
async function fetchJson(url) {
  ...
  try {
    ...
    throw new Error("map provider returned non-json response");
  }
