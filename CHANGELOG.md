...
## 2026-07-13：云端唯一同步护栏、Smoke test 降噪与文档导入

### 核心修复

- 新增 `cloud-sync-guard.js`，在协作主程序启动前运行。
- 云端房间的 WebSocket `join` 不再携带浏览器本地行程状态，避免旧设备或本地空白缓存把服务器房间初始化成错误数据。
- 阻止页面启动初期的“默认空白行程库”被当作有效状态写入服务器，降低不同设备互相覆盖、行程被吞或生成无意义 list 的风险。
- 保留现有服务器端实体级合并和 tombstone 删除保护；不自动删除任何历史行程单或历史备份。
- 继续保留 `local-recovery.js`，可把之前被本地缓存隐藏的行程恢复为新的 `本机恢复 - ...` list，恢复过程不覆盖云端当前数据。

### Smoke test

- GitHub Actions `Smoke test` 增加 `concurrency`，同一分支的新提交会取消旧的未完成运行，减少旧失败通知反复弹出。
- Smoke test 新增检查：

```text
./cloud-sync-guard.js?v=render-20260713h
./document-import.js?v=render-20260713i
render-20260713i
```

- Smoke test 继续检查 runtime 与直接 `node server.js` 两种 Render 启动路径。

### 文档导入与快速规划

- 新增 `document-import.js`，集成在快速规划区域。
- 快速规划区域新增“文档导入”和“收起 / 展开”按钮。
- 文档导入支持粘贴文本，或上传 `.txt`、`.md`、`.json`、`.html`、`.csv`。
- 导入默认生成一个新的 list，不覆盖当前 list，不复用原始实体 ID，避免同步冲突。
- 如果导入的是此前导出的 JSON / HTML，会优先读取其中的行程数据。
- 如果 Render 配置了 DeepSeek，导入时会先尝试用 `/api/ai/recommend` 辅助整理；失败时自动回退到本地文本解析。
- 本地解析支持：
...
- 不涉及 Render Persistent Disk 数据迁移。
- 不修改或删除 `/var/data/rooms.json` 中已有行程。
- 需要 Render 部署最新 `main` 后生效。
- 首页资源版本更新为 `render-20260713i`。

### 主要提交

- `aad1e59`：新增云端同步启动护栏。
- `8178fd2`：页面在 `app-collab.js` 前加载同步护栏。
- `8101f02`：Smoke test 增加并发取消和同步护栏检查。
- `60efb6d`：新增文档导入增强层。
- `4dad4d7`：页面加载文档导入脚本并更新版本号。
- `67dbef9`：Smoke test 检查文档导入脚本。

## 2026-07-12：同步稳定、固定事项编辑器与路线交互修复

### 核心修复

- 移除前端自动生成 `冲突备份 - ...` 的机制，正常多设备编辑不再被当作冲突备份写回服务器。
- 服务端改为按实体 ID 合并 list、day、activity、member 和 budget 数据，并使用 `updatedAt` / `orderUpdatedAt` 判断新旧。
- 删除操作记录 tombstone，防止旧设备或旧缓存把已经删除的 list、日期、事项、同行人重新写回来。
- 首次进入房间时服务器数据仍是权威数据源，本地缓存不再自动反推覆盖服务器。
- 历史冲突备份 list 默认保留并标记为历史备份，不自动删除，避免误删用户可能需要的数据。

### 编辑体验

- 新增固定悬浮事项编辑器，点击日卡片 `+`、事项编辑按钮、交通 chip、预算 chip 都打开同一个编辑器。
- 编辑器固定悬浮在页面上，不再插入日卡片内部，避免点击添加后页面抖动。
- 支持选中 day / activity / field，选中内容用绿色描边提示。
- 事项编辑器包含时间、标题、地点、备注、交通、预算、标签，保存仍走统一 `TripPlanner.saveExternalLibrary` 通道。
- 删除同行人面板下方重复的在线成员显示，只保留可管理的同行成员列表。
- 页面标题和浏览器标题改为“行程编辑器”。

### 地图与路线

- 地图和 AI 面板新增收起 / 展开按钮。
- 地图不再因为展开或收起某一天自动重新计算路线；必须点击“计算路线”才请求地图接口。
- 当前 list 地图只使用当前行程单的地点，不再混入其他 list。
- 路线摘要新增收起按钮；有对应事项的路线段新增“修改”按钮，可直接打开该事项的交通编辑。
- 修复地图工具栏安装和状态同步之间的递归调用，避免前端异常和重复 UI 刷新。
- 最近一次路线结果暴露给导出功能，用于导出路线摘要和 Google Maps 链接。

### 导入导出

- 左侧行程单区域新增“导出 / 导入”按钮。
- 导出当前 list 为本地 HTML 文件，包含天数、住宿、事项、交通、预算、标签和已计算路线摘要。
- 导出的 HTML 内嵌 JSON，可再次导入。
- 导入默认追加为新 list，并重新生成 list/day/activity/member ID，不覆盖当前 list，也避免同步实体 ID 冲突。
- 导入纯文本时会尽量解析为批量行程草稿。

### 部署影响

- 不修改 Render Persistent Disk 上已有 `rooms.json` 数据。
- 不会主动删除已有冲突备份；用户可后续手动清理。
- 需要 Render 重新部署最新 `main` 后生效。
- 首页资源版本更新为：

```text
./map-providers.js?v=map-20260712b
./map-providers.css?v=map-20260712b
./entry-actions.js?v=entry-20260712c
./entry-actions.css?v=entry-20260712c
```

### 主要提交

- `e4c29cb`：服务端改为实体级合并和 tombstone 删除保护。
- `13f3c72`：前端移除自动冲突备份，统一服务器权威同步。
- `47d4d67`：新增固定悬浮事项编辑器与导入导出入口。
- `4fe1af0`：修复地图工具栏递归、路线手动计算和路线编辑入口。
- `fc90b40`：页面标题改为“行程编辑器”并更新资源版本。
- `8e2f99b`：更新 smoke test 资源断言。
